...
$('#pagination').on('click', 'a', function(e) { // When click on a 'a' element of the pagination div
	...
	$('#staffList').html('<img src="/doa/img/loader-small.gif" alt=""/>'); // Display a processing icon
	var data = {page: page, per_page: 12, staff_info: info}; // Create JSON which will be sent via Ajax
	// We set up the per_page var at 4. You may change to any number you need.

	var serverUrl='/doa/unsecure/pagination.php';
	...
	$.ajax({ // jQuery Ajax
		...
		success: function(data) {
			$('#staffList').html(data.staffList); // We update the staffList DIV with the article list
